...
export async function createBooking(ctx: Context) {
  try {
    ...
    if (
      !guest || guest.user_id === undefined ||
      guest.points_balance === undefined
    ) {
      ctx.response.status = 401;
      ctx.response.body = {
        error: "User not authenticated properly or missing user data.",
      };
      return;
    }

    const body = ctx.request.body({ type: "json" });
    const { listing_id, check_in_date, check_out_date, num_guests } = await body
      .value;
    ...
    if (!listing_id || !check_in_date || !check_out_date || !num_guests) {
      ...
      ctx.response.body = {
        error:
          "Missing required fields: listing_id, check_in_date, check_out_date, num_guests.",
      };
      return;
    }
    ...
    if (!areDatesValid(check_in_date, check_out_date)) {
      ...
      ctx.response.body = {
        error:
          "Invalid dates. Ensure check-in is not in the past and check-out is after check-in.",
      };
      return;
    }
    ...
    if (num_guests > (listing.max_guests || 1)) {
      ...
      ctx.response.body = {
        error:
          `Number of guests exceeds the maximum allowed for this listing (${listing.max_guests}).`,
      };
      return;
    }
    ...
    if (isBookingOverlap(listing_id, newCheckInDate, newCheckOutDate)) {
      ...
      ctx.response.body = {
        error:
          "The listing is not available for the selected dates (overlap with existing booking).",
      };
      return;
    }
    ...
    const nights = Math.round(
      Math.abs((newCheckOutDate.getTime() - newCheckInDate.getTime()) / oneDay),
    );
    if (nights <= 0) {
      ctx.response.status = 400;
      ctx.response.body = {
    ...
    if (guest.points_balance < total_points_charged) {
      ...
      ctx.response.body = {
        ...
        current_balance: guest.points_balance,
        points_needed: total_points_charged,
      };
      return;
    }
    ...
    if (!paymentTransaction) {
      ...
      ctx.response.body = {
        error: "Failed to process point transaction for booking payment.",
      };
      return;
    }
    ...
    ctx.response.body = {
      ...
      booking: newBooking,
      new_balance: guestUserRecord.points_balance,
    };
  } catch (error) {
    ...
    if (
      ...
      ctx.response.status = 400;
      ctx.response.body = {
        error: "Invalid request body. Expected JSON with booking details.",
      };
    }
  }
}
...
export async function getGuestBookings(ctx: Context) {
  try {
    ...
    const guestBookings = bookings_db.filter((b) =>
      b.guest_id === guest.user_id
    );
...
export async function getListingBookings(ctx: Context) {
  try {
    ...
    if (listing.admin_id !== user.user_id) {
      ...
      ctx.response.body = {
        error: "You are not authorized to view bookings for this listing.",
      };
      return;
    }

    const listingBookings = bookings_db.filter((b) =>
...
export async function cancelBooking(ctx: Context) {
  try {
    ...
    if (booking.status === newStatus) {
      ...
      ctx.response.body = {
        message: "Booking is already in this state.",
        booking,
      };
      return;
    }
    ...
    if (
      wasConfirmed &&
      (newStatus === "cancelled_by_guest" || newStatus === "cancelled_by_host")
    ) {
      ...
      if (!refundTransaction) {
        console.error(
          `Failed to process point refund transaction for booking ${booking.id}. User ID: ${booking.guest_id}`,
        );
        ...
        ctx.response.body = {
          message:
            "Booking cancelled, but point refund processing encountered an issue. Please contact support.",
          booking,
        };
        return;
      }
