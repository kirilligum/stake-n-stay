...
export function addPointTransaction(
  ...
  if (!user) {
    console.error(
      `User with ID ${user_id} not found. Cannot add point transaction.`,
    );
    return null;
  }
  ...
  console.log(
    `User ${user.username} new balance after transaction: ${user.points_balance}`,
  );
...
export async function getMyPointTransactions(ctx: Context) {
  try {
    ...
    const userTransactions = point_transactions_db.filter((pt) =>
      pt.user_id === user.user_id
    );
...
export async function grantPointsToUser(ctx: Context) {
  try {
    ...
    if (target_user_id === undefined || points_to_grant === undefined) {
      ...
      ctx.response.body = {
        error: "target_user_id and points_to_grant are required.",
      };
      return;
    }
    ...
    if (typeof points_to_grant !== "number" || points_to_grant <= 0) {
      ctx.response.status = 400;
      ctx.response.body = {
        error: "points_to_grant must be a positive number.",
      };
      return;
    }
    ...
    if (!targetUser) {
      ...
      ctx.response.body = {
        error: `User with ID ${target_user_id} not found.`,
      };
      return;
    }

    const grantDescription = description ||
      `Admin grant by ${adminUser.username} (ID: ${adminUser.user_id})`;
    ...
    if (transaction) {
      ...
      ctx.response.body = {
        ...
        transaction,
        target_user_new_balance: users.find((u) =>
          u.id === Number(target_user_id)
        )?.points_balance,
      };
    } else {
    ...
    if (
      ...
      ctx.response.body = {
        error:
          "Invalid request body. Expected JSON with target_user_id and points_to_grant.",
      };
    }
  }
}
